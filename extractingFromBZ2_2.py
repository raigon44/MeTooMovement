...
def process_list_executor(process_func, process_lst, csv_writer, max_workers, raise_error):
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        result = executor.map(process_func, process_lst)
        ...
        with bz2.open(data_file, "r") as bzinput:
            ...
            for i, line in enumerate(io.BufferedReader(bzinput, buffer_size=buffer_size)):
                ...
                if len(quote_ids) > process_list_max_size:
                    result_count += process_list_executor(process_func, zip(quote_ids, line_lst), csv_writer, max_workers, raise_error)
                    break
                    quote_ids = []
                    line_lst = []
...
if __name__ == '__main__':
    ...
    speakers = []
    with open('speaker.txt', 'r', encoding='latin-1') as fp:
        for spk in fp:
            speakers.append(spk)
    ...
    process_quotebank_data(
        ...
        output_file='article_centric.csv',
